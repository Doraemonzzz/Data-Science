...
def r_squared(y, estimated):
    ...
    #import numpy as np
    y1=np.array(y)
    e1=np.array(estimated)
    s1=((y1-e1)*(y1-e1)).sum()/len(y1)
    s2=np.var(y1)
    R2=1-s1/s2
    return R2
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(m):
        predict=[]
        coe=models[i]
        predict=np.polyval(coe,x)
        pylab.plot(x,predict,'r',label='Predict values')
...
print(r_squared(y,np.polyval(models[0],x)))
